...
function SymbolCard({ symbol, index, visible, onExplore }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: 7,
        boxShadow: `0 0 16px ${symbol.color}16`,
      }}>
        <SymbolSVG type={symbol.type} color={symbol.color || '#c8a455'} active={drawn} />
      </div>

      <div style={{ flex: 1, minWidth: 0 }}>
        <span style={{ fontSize: '0.58rem', letterSpacing: '0.16em', color: '#555' }}>
          {String(index + 1).padStart(2, '0')}
        </span>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem', margin: '0.1rem 0 0.3rem' }}>
          <span style={{ fontSize: '1.2rem', fontWeight: 300, color: symbol.color || '#c8a455',
            textShadow: `0 0 18px ${symbol.color}44` }}>
            {symbol.name_zh}
          </span>
          <span style={{ fontSize: '0.68rem', color: '#555' }}>{symbol.name_en}</span>
        </div>
        <p style={{ fontSize: '0.72rem', color: '#888', lineHeight: 1.65, margin: '0 0 0.7rem' }}>
          {symbol.meaning_zh}
        </p>
        ...
        <button
          ...
          style={{
            ...
            border: `1px solid ${symbol.color}40`,
            borderRadius: 6,
...
export default function SymbolPanel({ artwork, onExplore }) {
  ...
  const analyze = useCallback(async () => {
    ...
    setLoading(true); setError(''); setSymbols(null); setNeedSetup(false);
    try {
      const { data, error: fnErr } = await supabase.functions.invoke('analyze-symbols', {
        body: { imageURL: artwork.originalURL },
      });
      if (fnErr) throw fnErr;
      if (data?.error?.includes('not configured')) { setNeedSetup(true); return; }
      if (data?.error) throw new Error(data.error);
      setSymbols(data?.symbols ?? []);
    } catch (err) {
      const msg = err?.message || String(err);
      if (msg.includes('not configured') || msg.includes('503')) setNeedSetup(true);
      else setError(msg);
    } finally { setLoading(false); }
  }, [artwork.id, artwork.originalURL, symbols, needSetup]);

  const handleOpen = () => { setOpen(true); analyze(); };
  ...
  return (
    <>
      ...
      <button onClick={open ? () => setOpen(false) : handleOpen} style={{
        ...
        background: open ? 'rgba(200,164,85,0.18)' : 'rgba(200,164,85,0.10)',
        border: '1px solid rgba(200,164,85,0.35)', borderRadius: 99,
        color: '#c8a455', fontSize: '0.65rem', letterSpacing: '0.18em',
        padding: '0.42rem 1.1rem', cursor: 'pointer',
        backdropFilter: 'blur(8px)', transition: 'all 0.25s',
        display: 'flex', alignItems: 'center', gap: '0.45rem', zIndex: 20,
      ...
      <div style={{
        ...
        <div style={{ padding: '1.1rem 1.2rem 0.8rem', borderBottom: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.15rem' }}>
            <span style={{ color: '#c8a455', fontSize: '0.72rem' }}>☯</span>
            <h3 style={{ fontSize: '0.68rem', letterSpacing: '0.2em', color: '#ddd', margin: 0, fontWeight: 400 }}>
              传统文化符号
            </h3>
          </div>
          <p style={{ fontSize: '0.6rem', color: '#444', margin: 0 }}>{artwork.title}</p>
        </div>

        <div style={{ flex: 1 }}>
          {loading && (
            <div style={{ padding: '3rem 1.5rem', textAlign: 'center' }}>
              <div style={{ width: 48, height: 48, margin: '0 auto 1rem', borderRadius: '50%',
                border: '1.5px solid #c8a455', animation: 'inkPulse 1.8s ease-in-out infinite' }} />
              <p style={{ fontSize: '0.7rem', color: '#555', letterSpacing: '0.16em' }}>解析传统文化符号...</p>
            </div>
          )}

          {needSetup && (
            <div style={{ padding: '1.8rem 1.2rem', textAlign: 'center' }}>
              <p style={{ fontSize: '0.8rem', color: '#777', marginBottom: '1rem', lineHeight: 1.6 }}>
                需要配置 Anthropic API Key
              </p>
              <div style={{ background: 'rgba(200,164,85,0.08)', border: '1px solid rgba(200,164,85,0.2)',
                borderRadius: 8, padding: '0.9rem', textAlign: 'left', fontSize: '0.65rem',
                color: '#666', fontFamily: 'monospace', lineHeight: 2 }}>
                supabase secrets set<br />ANTHROPIC_API_KEY=sk-ant-...
              </div>
            </div>
          )}
          ...
          {error && !needSetup && (
            <div style={{ padding: '2rem 1.2rem', textAlign: 'center' }}>
              <p style={{ fontSize: '0.75rem', color: '#844', marginBottom: '1rem' }}>{error}</p>
              <button onClick={() => { lastAnalyzed.current = null; analyze(); }}
                style={{ background: 'none', border: '1px solid #555', borderRadius: 6,
                  color: '#777', fontSize: '0.65rem', padding: '0.35rem 0.8rem', cursor: 'pointer' }}>
                重试
              </button>
            </div>
          )}
          ...
          {symbols?.length === 0 && !loading && (
            <div style={{ padding: '2.5rem', textAlign: 'center', color: '#444', fontSize: '0.75rem' }}>
              未识别出传统文化符号
            </div>
          )}
          ...
          {symbols?.map((sym, i) => (
            <SymbolCard key={i} symbol={sym} index={i} visible={open}
              onExplore={(idx) => onExplore(symbols, idx)} />
          ))}
        </div>

        {symbols?.length > 0 && (
          <div style={{ padding: '0.8rem 1.2rem', borderTop: '1px solid rgba(255,255,255,0.05)',
            fontSize: '0.58rem', color: '#333', letterSpacing: '0.1em', flexShrink: 0 }}>
            AI 解析 · 仅供参考
          </div>
        )}
      </div>
